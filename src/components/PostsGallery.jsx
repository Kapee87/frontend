        ...
        <div className='m-10 mb-20 flex flex-wrap gap-5 justify-center' id='postsGallery'>
            ...
            {posts?.map((post, index) => {
                ...
                return <div className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30 rounded-xl" key={post._id}>
                    ...
                    <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0 pb-14">
                        ...
                        <NavLink to={`/post/${post._id}`} className="rounded-full bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60">Ver Más</NavLink>
                        {/* 
                                //Analizar si ponemos este condicional para ver si el usuario además de estar logeado, es administrador
                         
                        {token ? <div className='h-14 w-14 absolute p-2 bottom-0 tooltip' data-tip='Eliminar'>
                            ❌
                        </div> : ''} 
